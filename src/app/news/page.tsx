...
import { Search, Clock, Calendar, BookOpen, ArrowRight, MessageSquare } from "lucide-react";
...
export default function NewsIndexPage() {
  ...
  const featuredArticle = useMemo(() => {
    return newsData.find((a) => a.isFeatured) || newsData[0];
  }, []);

  const filteredArticles = useMemo(() => {
    return newsData.filter((art) => {
      const matchCat = selectedCategory === "Tất cả tin tức" || art.category === selectedCategory;
          ...
          <div className="flex flex-col md:flex-row items-center justify-between gap-4 p-4 rounded-3xl bg-white border border-slate-200 shadow-lg">
            ...
            <div className="relative w-full md:w-72 shrink-0">
              ...
              <input 
                ...
                className="w-full pl-10 pr-4 py-2.5 rounded-xl bg-slate-50 border border-slate-200 text-xs text-slate-900 focus:outline-none focus:border-brand transition-all"
              />
            </div>
          </div>
