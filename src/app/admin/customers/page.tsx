      ...
      <div className="bg-white p-4 rounded-2xl border border-slate-200/80 shadow-xs flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4">
        ...
        <div className="flex items-center gap-2 w-full sm:w-auto">
          <select
            ...
            className="w-full sm:w-auto px-3 py-2 text-xs font-semibold border border-slate-200 rounded-xl bg-white text-slate-700 focus:outline-none focus:ring-2 focus:ring-red-500/20"
          >
          ...
          {filteredCustomers.map((customer) => (
            <div key={customer.id} className="p-4 space-y-3 hover:bg-slate-50/50 transition-colors">
              <div className="flex items-start justify-between gap-2">
                ...
                <span
                  className={`inline-block px-2.5 py-0.5 rounded-full text-[10px] font-extrabold border flex-shrink-0 ${customer.typeBadge}`}
                >
                  {customer.type}
                </span>
              </div>

              <div className="p-2.5 rounded-xl bg-slate-50 border border-slate-200/60 grid grid-cols-2 gap-2 text-xs">
                <div>
                  <span className="text-slate-400 text-[11px] block">Người đại diện:</span>
                  <span className="font-bold text-slate-800">{customer.contactPerson}</span>
                </div>
                <div>
                  <span className="text-slate-400 text-[11px] block">Khu vực:</span>
                  <span className="font-medium text-slate-700">{customer.location}</span>
                </div>
              </div>

              <div className="flex items-center justify-between pt-1 text-xs">
                <div>
                  <span className="font-extrabold text-slate-900 block">{customer.totalOrders} đơn báo giá</span>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <a
                    ...
                    className="px-3 py-1.5 rounded-lg bg-emerald-50 text-emerald-600 font-bold text-xs flex items-center gap-1"
                  >
