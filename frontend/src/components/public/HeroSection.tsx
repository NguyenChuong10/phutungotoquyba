...
export default function HeroSection() {
      ...
      try {
        ...
        if (res && (res.ok || res.success) && Array.isArray(res.data) && res.data.length > 0) {
          const items: SlideItem[] = res.data.map((s: any) => ({
            id: s.id,
            bgImage: formatImageUrl(s.imageUrl),
  ...
  if (slides.length === 0) {
    if (loading) {
      return (
        <section className="relative w-full h-[95vh] min-h-[700px] max-h-[1000px] flex items-center justify-center bg-slate-950 animate-pulse">
          <div className="text-slate-500 font-bold text-sm">Đang tải banner Q.BA...</div>
        </section>
      );
    }
    return null;
  ...
  return (
    <section className="relative w-full h-[95vh] min-h-[700px] max-h-[1000px] flex items-center justify-center overflow-hidden bg-slate-950">
      ...
      {slides.map((slide, index) => (
        <div
          key={slide.id}
          className={`absolute inset-0 z-0 transition-opacity duration-1000 motion-reduce:transition-none ${index === currentSlide ? 'opacity-100' : 'opacity-0'
            }`}
        >
        ...
        <h2 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-black italic uppercase tracking-wider font-heading leading-tight flex flex-wrap items-center gap-x-3 gap-y-1">
          ...
          <span className="relative inline-block min-w-[200px] sm:min-w-[260px] md:min-w-[360px] h-[1.3em]">
            {slides.map((slide, index) => (
              <span
                key={`cat-${slide.id}`}
                className={`absolute left-0 top-0 text-[#FF0000] drop-shadow-[0_4px_14px_rgba(0,0,0,0.95)] whitespace-nowrap transition-all duration-700 ease-out pr-4 inline-block ${index === currentSlide
